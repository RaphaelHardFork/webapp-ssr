...
// endregion:	=== Generation & validation ===
...
mod tests {
    ...
    fn test_token_display_ok() -> Result<()> {
        ...
        let fx_token_str =
            "ZngtaWRlbnQtMDE.MjAyMy0wNS0xN1QxNTozMDowMFo.some-sign-b64u-encoded".to_string();
    ...
    fn test_token_from_str_ok() -> Result<()> {
        ...
        // assert_eq!(format!("{token:?}"), format!("{fx_token:?}")); // => Without partial Eq
    ...
    fn test_validate_web_token_err_expired() -> Result<()> {
        ...
        assert!(
            ...
            "Should have matched `Err(Error::TokenExprired but was `{res:?}`)`"
        );
